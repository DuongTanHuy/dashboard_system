...
import {
  affiliate_chart_data,
  affiliate_compare_summary_data,
  affiliate_summary_data,
} from 'src/utils/mock';
...
const SUMMARY_ITEM = [
  {
    id: 'total_users_register_ref',
    name: 'Tổng người được giới thiệu',
    color: COLORS[0],
    icon: 'fluent:person-link-20-filled',
    data: [],
    total: 0,
    display: true,
  },
  {
    id: 'total_users_become_affiliate',
    name: 'Tổng số người tham gia Affiliate',
    color: COLORS[4],
    icon: 'mdi:user-group',
    data: [],
    total: 0,
  ...
  {
    id: 'total_commission_balance_unpaid',
    name: 'Tổng số hoa hồng chưa thanh toán',
    color: COLORS[5],
...
export default function AffiliateStatisticalView() {
  ...
  useEffect(() => {
    const handleFetchTotalData = async () => {
      try {
        ...
        // api not available
        // const response = await getAffiliateSummaryApi(params);
        const data = affiliate_summary_data;
        setSummaryItem((prev) =>
          prev.map((item) => ({
            ...item,
    ...
    const handleFetchCompareData = async () => {
      try {
        ...
        // api not available
        // const response = await getAffiliateSummaryApi(params);
        setCompareSummary(affiliate_compare_summary_data);
      } catch (error) {
        console.log(error);
      }
    };
  ...
  useEffect(() => {
    const fetchChartData = async () => {
      try {
        ...
        // api not available
        // const response = await getAffiliateChartApi(params);
        const { categories: category, data } = affiliate_chart_data;
      ...
      <Stack spacing={3}>
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12}>
            <AnalyticChart
              dataType="money"
              title="Biểu đồ thống kê hoa hồng"
              subheader={`Thống kê theo thời gian: ${shortDateLabel(
                rangeCalendarPicker.startDate,
                rangeCalendarPicker.endDate,
                "d 'tháng' M yyyy",
